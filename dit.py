...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, c):
        ...
        # print("x before gate_msa: ", x)
        x = x + gate_msa.unsqueeze(1) * self.attn(
            modulate(self.norm1(x), scale_msa, shift_msa)
        )
        # print("x after gate_msa: ", x)
...
class DiT(nn.Module):
    ...
    def forward(self, x, r, t, y):
        ...
        """
        # print(f'input shapes:{x.shape} {r.shape} {t.shape} {y.shape}')
        ...
        c = t + y
        # print(f'x shape {x.shape} and c.shape {c.shape}')                                # (N, D)
        for i, block in enumerate(self.blocks):
            # print(f"x {i} shape {x} and c {i} shape {c.shape}")
            x = block(x, c)
